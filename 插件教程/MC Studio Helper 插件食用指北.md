...
直接打开项目根目录的 `.mcdev.json`，即可使用「可视化编辑」和「JSON 原文」两个页面。Run 页面提供「打开 .mcdev.json」快捷入口；文件不存在时创建一个空 JSON 对象，表单会显示内置默认值。已有文件及尚未保存的修改不会因打开操作被重写。
...
切换 **可视化编辑 / JSON 原文** 可直接粘贴完整 JSON 替换。格式或值无效时保留输入并提示修正，不能切回表单；JSON 原文遵循 IDE 的保存行为，启动前会校验配置。原生文本编辑器仍可通过文件编辑器标签进入，支持 JSON 高亮、撤销和重做。文件编辑使用同一个 IDE 文档，遵循 IDE 的保存及自动保存行为。
